...
function Hero() {
  return (
    <section id='hero' className='relative w-full min-h-screen flex flex-col items-center justify-center overflow-hidden' aria-label='Hero Section'>
      {/* Background image placeholder — replace src when ready */}
      <div className='absolute inset-0 bg-cover bg-center bg-no-repeat' style={{ backgroundImage: `url(${heroBg})` }} aria-hidden='true' />

      {/* Dark overlay for readability over the future background image */}
      <div className='absolute inset-0 bg-gradient-to-b from-[#7a0a0a]/80 via-[#7a0a0a]/70 to-black/60' aria-hidden='true' />
      ...
      <div className='relative z-10 flex flex-col items-center text-center px-4 sm:px-8 lg:px-16 max-w-5xl mx-auto py-24'>
        ...
        {/* Subtitle */}
        <p className='text-base sm:text-lg md:text-xl font-semibold text-white/90 mb-3'>Life-Saving Care at Your Doorstep</p>
        <p className='text-sm sm:text-base md:text-lg text-white/75 max-w-2xl leading-relaxed'>
          Swift, Reliable, and Professional Ambulance Services 24/7. Our dedicated team is here to provide immediate medical assistance and transport,
          ensuring your safety and comfort in critical moments.
        </p>
        ...
        <div className='mt-10 flex flex-col sm:flex-row items-center gap-3'>
          <a
            href='#contact'
            id='hero-contact-btn'
            className='inline-flex items-center justify-center px-8 py-3.5 rounded-full bg-white/10 hover:bg-white/20 text-white border border-white/30 font-semibold text-base sm:text-lg shadow-md transition-all duration-200 focus:outline-none focus:ring-4 focus:ring-white/30'
          >
            Contact Us
          </a>
        </div>
        ...
        {/* Scroll Indicator */}
        <div className='mt-10 hidden sm:flex items-center gap-2 text-white/80 text-sm'>
          <a
            href='#services'
            className='inline-flex items-center justify-center w-9 h-10 rounded-full bg-white/10 hover:bg-white/20 border border-white/20 transition-colors'
          >
            <svg
